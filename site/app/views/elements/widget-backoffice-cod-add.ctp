...
<div class="siteSetting-widget-container">
	<div class="content2-widget-container">
		...
		<div class="detail">
			...
			<?php	
				if(isset($widget_cod_add_result) && !$widget_cod_add_result)
				{
			?>
			...
			<?php
				...
				else if(isset($widget_cod_add_result) && $widget_cod_add_result)
				{
			?>
					...
					<td colspan="3">
						<?php
								echo $this->Js->submit(__('Create',true), array(
											'url' => "/{$this->params['prefix']}/cods/widget_cods_add/{$container}",
											'method'=> 'post',
			...
			<script>
				$(document).ready(function() {
					...
					$("#lnk-widget-cod-add-cancel").click(function(){
		    			<?php if($container == 'none'){?>
							$('#widget-main-mid-bottom-container').html('');
		    			<?php }else{?>
							$.get('<?php echo $_SESSION['Auth']['ManageCODURL'];?>', function(data){
