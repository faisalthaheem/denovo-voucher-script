...
class IcodesusMerchantsController extends Dvs4icodesusAppController {

	var $name = 'IcodesusMerchants';
	var $uses = array('IcodesusMerchant','Merchant','IcodesusCategory','Category');
	...
	function admin_mappingfilter($what = 'all')
	{
		...
		switch($what){
			case 'mapped':
				$filter = 'IcodesusMerchant.merchant_id IS NOT NULL';
				break;
			case 'unmapped':
				$filter = 'IcodesusMerchant.merchant_id IS NULL';
				break;
		}
	...
	function admin_map_content()
	{
		...
		if(isset($_SESSION[$this->__PLUGIN_NAME]['search']) && !empty($_SESSION[$this->__PLUGIN_NAME]['search']))
		{
			if(!empty($_SESSION[$this->__PLUGIN_NAME]['search']['filter'])){
				$conditions[] = "IcodesusMerchant.merchant LIKE '%{$_SESSION[$this->__PLUGIN_NAME]['search']['filter']}%'";
			}
		...
		$this->paginate['IcodesusMerchant']['limit'] = $limit;
		$this->paginate['IcodesusMerchant']['conditions'] = $conditions;
		$icMerchants = $this->paginate('IcodesusMerchant');
		
		foreach($icMerchants as &$icMerchant)
		{
			$icMerchant['IcodesusMerchant']['match'] = $this->__admin_map_suggest($icMerchant['IcodesusMerchant']['merchant']);
		}
